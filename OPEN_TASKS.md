# MemPalace Open Tasks

Last updated: 2026-05-10

This file is the durable local index for active `mempalace` issues.

## Active Issues

- [#1 - Adopt projects-ops repo bootstrap governance baseline](https://github.com/iMelki/mempalace/issues/1)
  - Goal: Align repo with the portfolio-wide governance and hygiene standards.
  - Status: In Progress (agent/antigravity/1-adopt-governance-baseline)

## Recently Completed

- (Initial baseline rollout)

## Delegated / Future

- (TBD)

## Supporting Docs

- [AGENTS.md](AGENTS.md)
- [CONTRIBUTING.md](CONTRIBUTING.md)
- [.github/labels.yml](.github/labels.yml)
